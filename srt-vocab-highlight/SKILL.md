...
- 释义表 `--glosses`：`{"序号": {"英文词": "中文释义"}}`，释义须为该行译文实际子串；颜色自动跟随对应英文生词
  - **释义值禁止写成英文原词自指**（如 `{"fit": "fit"}`、`{"well fit": "well fit"}`）：若纠正行里该英文词恰好原样出现，释义子串会在已被英文着色的文字上再包一层 `<font>`，产生 `嵌套=2` 类错误。中文行无对应释义词时**宁可不设该释义**（英文词本身仍会被英文高亮覆盖），或改用行内真实存在的中文词（如 fit→迷人、well fit→超帅）
  - **释义值禁止传空字符串 `""`**：脚本会 `gloss_color[""]=<颜色>` 然后 `re.compile("")`，空模式匹配每个位置，导致该 cue 中文行**每个字**都被包上 `<font></font>字</font>`，且嵌套=2、视觉灾难。中文行不想上色时直接不传 `--glosses` 参数（或把整个 `--glosses` 文件落盘为 `{}`），不要逐词传空串
